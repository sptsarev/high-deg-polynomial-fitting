...
##### this version works with arbitrary precision arithmetic (BigFloat)
...
# <prec> - precision for BigFloat calculations (in bits)
...
# 256 means that all computations will be performed with precision 256 bits
############################################################
...
for ind_data = 2:length_DATA
    ...
    if abs((DATA[ind_data,1]-pva) - step_value_argument)>prec_toler
        println("warning: Non equidistant data, use the script polyn_approx_BigFloat_nonequidistant-en.jl")
        exit()
    end
